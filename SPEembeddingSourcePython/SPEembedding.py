...
class Embedding:
    ...
    def RecursivelyEmbed(parents, level, pathsDict, edgesDict, fixedCoordinate, coordinates):
        ...
        for parent in parents:
            children = Embedding.FindChildren(parent, level, pathsDict)
            if len(children) > 0:
                Embedding.RecursivelyEmbed(children, level+1, pathsDict, edgesDict, fixedCoordinate, coordinates)     
    ...
    def RecursivelyCreateDataFileAndFolders(allPoints, parentsKeys, level, startingFolder, pathsDict):#allPoints is a dict where keys are id's and values are Point objects    
        Embedding.CreateDataJSONFile(allPoints, parentsKeys, startingFolder)
        for parent in parentsKeys:
            childrenKeys = Embedding.FindChildren(parent, level, pathsDict)
            if len(childrenKeys) > 0:
                Embedding.CreateDirIfDoesNotExist(startingFolder + "/" + parent)
                Embedding.RecursivelyCreateDataFileAndFolders(allPoints, childrenKeys, level+1, startingFolder + "/" + parent, pathsDict)       
    ...
    def Workflow(simGraphFile, clusteringHierarchyFile, metaDataFile, namesOfPropertiesFile, propertiesIntensitiesFile, bigDataMode = "true"):
        ...
        Embedding.RecursivelyEmbed(roots, 0, pathsDict, edgesDict, fixedCoordinate, coordinates)
        ...
        Embedding.RecursivelyCreateDataFileAndFolders(pointsDict, roots, 0, "data", pathsDict)
